"""Training loop for the Concat+MLP fusion baseline (scripts/models/fusion/concat_baseline.py)."""
...
from scripts.training.fusion_train import _load_labels, _scale_hc, _accuracy
from scripts.models.fusion.concat_baseline import ConcatMLP
from scripts.utils.logging_utils import setup_logging

logger = logging.getLogger(__name__)

_ROBERTA_BASELINE_F1 = 0.8873
OUTPUT_DIR = config.RESULTS_DIR / "models" / "fusion" / "concat_mlp"
...
def train() -> dict:
    ...
    ckpt_dir = config.ARTIFACTS_DIR / OUTPUT_DIR.relative_to(config.RESULTS_DIR) / "checkpoints"
    ...
    criterion = nn.CrossEntropyLoss(label_smoothing=config.FUSION_LABEL_SMOOTHING)
